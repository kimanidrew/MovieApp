...
export default function HlsPlayer({
  ...
  const restoreProgress = (video: HTMLVideoElement) => {
    try {
      ...
      const applyTime = () => {
        video.currentTime = Math.min(saved.time, video.duration - 1 || saved.time);
      ...
      if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, (_, data) => {
          ...
          if (preferredHeight && hls) {
            ...
            const highest = available.length - 1;
  ...
  return (
    ...
    >
      ...
      {/* Interaction Overlay (Double Tap Logic) */}
          ...
          <div className="controls-right">
            {qualities.length > 0 && (
              <select className="quality-select" value={currentQuality} onChange={(e) => handleQualityChange(Number(e.target.value))}>
                ...
                {qualities.map(q => <option key={q.id} value={q.id}>{q.height}p</option>)}
              </select>
            )}
      ...
      <style>{`
        ...
        .back-button { color: white; background: transparent; cursor: pointer; border: none; display: flex; margin-right: 2rem; transition: transform 0.2s; }
        ...
        .quality-select { background: rgba(0,0,0,0.6); color: white; border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; padding: 4px; cursor: pointer; font-size: 0.8rem; }
